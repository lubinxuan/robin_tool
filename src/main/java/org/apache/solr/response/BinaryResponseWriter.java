...
public class BinaryResponseWriter implements BinaryQueryResponseWriter {
    ...
    public static class Resolver implements JavaBinCodec.ObjectResolver, JavaBinCodec.WritableDocFields {
        ...
        protected long[] writeResultsBody(ResultContext res, JavaBinCodec codec) throws IOException {
            ...
            long costCount = 0, writeCost = 0;
            while (docStreamer.hasNext()) {
                long s = System.nanoTime();
                SolrDocument doc = docStreamer.next();
                costCount += (System.nanoTime() - s);
                s = System.nanoTime();
                codec.writeSolrDocument(doc);
                writeCost += (System.nanoTime() - s);
            }
            return new long[]{costCount, writeCost};
        }
        ...
        public void writeResults(ResultContext ctx, JavaBinCodec codec) throws IOException {
            ...
            long[] cost = writeResultsBody(ctx, codec);

            LOG.error("读取doc耗时 {}   写出耗时 {}", cost[0], cost[1]);
        }
